...
NeoPixel::NeoPixel(int id, int num, int pin, int config) : AppAction(id), numLEDs(num), numBytes(num * 3), _pin(pin), type(config), pixels(NULL)
#ifdef __AVR__
...
void NeoPixel::OnExcute()
{
	...
	setPixelColor(_index, _red, _green, _blue);
	show();
}
...
// 8 MHz(ish) AVR ---------------------------------------------------------
  ...
  if((type & NEO_SPDMASK) == NEO_KHZ800)
  { // 800 KHz bitstream
...
*/
 ...
 #error "CPU SPEED NOT SUPPORTED"
  ...
  volatile uint8_t *clr = portClearRegister(_pin);
  #define SET_HI   *set = 1;
  #define SET_LO   *clr = 1;
  uint8_t *p   = pixels,
